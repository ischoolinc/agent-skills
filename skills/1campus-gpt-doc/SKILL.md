---
name: 1campus-gpt-doc
description: V4 Client API 使用指南。1Campus GPT V4 API 的第三方開發者文件，涵蓋 Request/Response 規格、SSE 串流事件、Client Function Call、Context 模板、錯誤處理、Skirk 嵌入整合。當討論 V4 API 呼叫方式、串流實作、事件處理、前端整合、API 請求格式、error codes、tool_outputs、二階段 API、EventSource、Skirk、認證模式（auth mode）、嵌入 AI 助理時使用此 Skill。
...
| Skirk 是什麼？怎麼建立助理？ | `skirk/quickstart.md` |
| Skirk 認證模式？auth mode？ | `skirk/auth-modes.md` |
| Skirk 嵌入？iframe？入口 URL？ | `skirk/embedding.md` |
...
### Skirk 嵌入整合
- `skirk/quickstart.md` - 快速入門（建立第一個 Skirk App）
- `skirk/auth-modes.md` - 認證模式完整指南（anonymous、identity_code、passthrough、code_exchange）
- `skirk/embedding.md` - 嵌入指南（入口 URL、iframe、LINE Bot）
